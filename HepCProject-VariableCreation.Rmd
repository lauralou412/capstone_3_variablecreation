...
## Age ##
library(eeptools)
patients$DOB_DS[is.na(patients$DOB_DS)] <- Sys.Date() ## do want to use today's date for unknown DOB??????
patients$AGE <- (age_calc(dob = patients$DOB_DS, enddate = Sys.Date(), units = "years"))  #calcuate age ##  do we want first encounter date to caclucate age frm that date instead of today's daate?####
...
####  DOESNT SEEM TO MAKEING ANY PATIENTS FROM CPT_UCPG TABLE????????????????
...
pts_diagnosis$BeforeorAfter <- ifelse(pts_diagnosis$First_Date_with_Hep_C_ICD_code < pts_diagnosis$Date, 0, 
                                      ifelse(pts_diagnosis$First_Date_with_Hep_C_ICD_code == pts_diagnosis$Date, 0, 1))
...
colnames(cpt_freq_table)[2:60] <- paste("FREQ", colnames(cpt_freq_table[,c(2:60)]), sep = "_")
