...
value_sets$Code<-as.factor(value_sets$Code)
...
codes_from_value_sets <- as.data.frame(reduced_value_sets$Code) #creating a dataset of codes only
codes_from_value_sets <- unique(codes_from_value_sets) #remove duplicates
codes_from_value_sets <- droplevels(codes_from_value_sets) #drop extra factor levels
colnames(codes_from_value_sets) <- ("Code")#rename column
...
pt_icd_codes <- pts_diagnosis %>%
                  select(PATIENT_ID, Code = icd_dx )
str(pt_icd_codes)
...
icd_freq_table <- icd_freq_table[c(177, 1:176)]

#rename column names to icd_code_freq for columns 2:98
colnames(icd_freq_table)[2:177] <- paste("FREQ", colnames(icd_freq_table[,c(2:177)]), sep = "_")

####  Binning icd codes to reduce number of variables ###

#create a vector of names for the reduced number categories ##
icd_names <- names(icd_freq_table[,2:177])
icd_names <- gsub("\\..*", "", icd_names)
icd_names <- unique(icd_names)

##function to add similar rows together
binned_icd_freq_table <- as.data.frame(sapply(icd_names, function(x) rowSums(icd_freq_table[,grep(x, names(icd_freq_table)), drop=FALSE])))
str(binned_icd_freq_table)

binned_icd_freq_table <- cbind(PATIENT_ID = icd_freq_table$PATIENT_ID, binned_icd_freq_table)

### Validating function that it worked###
test <- as.data.frame(icd_freq_table$FREQ_131.01 + icd_freq_table$FREQ_131.09 + icd_freq_table$FREQ_131.8 + icd_freq_table$FREQ_131.9)
colnames(test) <- c("FREQ_131")

test$compare <- ifelse(test$FREQ_131 == binned_icd_freq_table$FREQ_131, "SAME", "DIFFERENT")
sum(nrow(subset(test, compare == "DIFFERENT")))

dim(icd_freq_table)
dim(binned_icd_freq_table)
icd_freq_table[4000,]
binned_icd_freq_table[4000,]

rowSums(icd_freq_table[578,2:177])
rowSums(binned_icd_freq_table[578, 2:41])

#join back with all patients that were lost; 
all_pts_binned_icd_freq_table <- left_join(patients, binned_icd_freq_table)

# chanage NA's to zeros
all_pts_binned_icd_freq_table[,5:44][is.na(all_pts_binned_icd_freq_table[,5:44])] <- 0
