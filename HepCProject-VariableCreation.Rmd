...
colnames(icd_freq_table)[2:57] <- paste("FREQ_ICDh", colnames(icd_freq_table[,c(2:57)]), sep = "_")#rename column names to icd_code_freq for columns 2:56
...
table(encounters$PATIENT_ID, encounters$Zip_Code_DID)
...
pts_zip_ACS <- left_join(pts_zip_ACS, ACS, c("Most_Freq_Zip" = "Zip_Code_DID")) #add ACS by matching on zip code; 954 zip codes unmatched???
patients <- left_join(patients, pts_zip_ACS) #join with other patient variables

dim(pts_zip_ACS)
sum(is.na(patients$Most_Freq_Zip))  #25304 NA's - no zip code
sum(is.na(patients$Pct_No_Health_Insurance)) #26258 NA's - no ACS data
###What to with NA's???

nrow(subset(patients, is.na(Most_Freq_Zip) & HCV_STATUS == 0))  #25079
nrow(subset(patients, is.na(Most_Freq_Zip) & HCV_STATUS == 1))  #225

str(patients[,151:180])
```
...
```
```{r}
#####  Double Check flat File

sum(is.na(patients[,172]))  # should be zero NA's
#26258
dim(patients)  #232661   180
str(patients)
names(patients)

patients[,131:172][is.na(patients[,131:172])] <- 0 
```
