...
hep_c_project_HEDIS_procedure_codes <- subset(hep_c_project_HEDIS_value_sets, Code_System == "ICD9PCS" | Code_System == "ICD10PCS" | Code_System == "CPT" | Code_System == "HCPCS") #only proecure codes only
...
#hep_c_project_HEDIS_procedure_codes$Code <- str_pad(hep_c_project_HEDIS_procedure_codes$Code, 4, pad = "0") #pad so all codes are 4 characters long
hep_c_project_HEDIS_procedure_codes <- hep_c_project_HEDIS_procedure_codes[!duplicated(hep_c_project_HEDIS_procedure_codes$Code),] #remove duplicates icd;

str(hep_c_project_HEDIS_procedure_codes)
...
#identify which observations occurred after First_Date_with_Hep_C_ICD_code
pts_cpt$BeforeorAfter <- ifelse(pts_cpt$First_Date_with_Hep_C_ICD_code < pts_cpt$Date, 0, 
...
cpt_freq_table <- cpt_freq_table[c(86, 1:85)] #move PATIENT_ID column to columns 1
colnames(cpt_freq_table)[2:86] <- paste("FREQ", colnames(cpt_freq_table[,c(2:86)]), sep = "_") #rename column names to icd_code_freq for columns 2:98
...
# chanage NA's to zeros
all_pts_cpt_freq_table[,4:88][is.na(all_pts_cpt_freq_table[,4:88])] <- 0
