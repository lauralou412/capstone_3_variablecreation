...
unique_pt_id_all_tbls<-as.data.frame(unique(c(cpt$PATIENT_ID,demo$PATIENT_ID,diagnosis_icd$PATIENT_ID,education$PATIENT_ID,encounters$PATIENT_ID,procedure_icd$PATIENT_ID,social_hx$PATIENT_ID)))
...
patients <- patients[,c( "PATIENT_ID", "First_Date_with_Hep_C_ICD_code", "HCV_STATUS", "DOB_DS", "Gender", "Race", "Ethnicity", "language", "Marital_Status")] #reducing down the columns
names(patients)[c(4:9)] <- c("DOB","GENDER", "RACE", "ETHNICITY", "LANGUAGE", "MARITAL_STATUS") #rename columns



## Remove NA's
patients[,"RACE"][is.na(patients[,"RACE"])] <- "Unknown"
patients[,"GENDER"][is.na(patients[,"GENDER"])] <- "U"
...
library(eeptools)
## Age ##
patients$DOB[is.na(patients$DOB)] <- Sys.Date()
patients$AGE <- age_calc(dob = patients$DOB, enddate = Sys.Date(), units = "years")

## Language ##
## reduce Language levels to 2 - NOT WORKING
...
#levels(patients$LANGUAGE) <- c(patients$LANGUAGE, "Unknown")
patients[,"LANGUAGE"][is.na(patients[,"LANGUAGE"])] <- "Non-English"
table(patients$LANGUAGE)
...
hep_c_project_HEDIS_procedure_codes <- subset(hep_c_project_HEDIS_value_sets, Code_System == "CPT" | Code_System == "ICD10PCS" | Code_System == "ICD9PCS") #only Procedure codes only #356 codes

```

```{r}
###  Join patient id table to diagnosis_icd table to bring in first Hep C date
pts_diagnosis <- left_join(patients, modified_diagnosis_icd) #8,479,127 rows
dim(pts_diagnosis)
str(pts_diagnosis)
...
pt_icd_codes_value_sets <- inner_join(hep_c_project_HEDIS_icd_codes, pt_icd_codes, by = c("Code" = "icd_dx"))
dim(pt_icd_codes)
...
patients[,9:65][is.na(patients[,9:65])] <- 0  #changed this line!!!!!!
...
cpt_freq_table <- cpt_freq_table[c(81, 1:80)]#move PATIENT_ID column to columns 1
rownames(cpt_freq_table) <- NULL#Remove row names from data.frame

#rename column names to icd_code_freq for columns 2:98
colnames(cpt_freq_table)[2:81] <- paste("FREQ", colnames(cpt_freq_table[,c(2:81)]), sep = "_")
...
# chanage NA's to zeros
patients[,66:145][is.na(patients[,66:145])] <- 0

######HH - add this to the patients table
...
patient_ID_encounters<-data.frame(table(encounters_first_hep_c_encounter$PATIENT_ID[encounters_first_hep_c_encounter$before_diagnosis=="TRUE"]))
...
patient_ID_encounters_outpt <- data.frame(table(encounters_first_hep_c_encounter$PATIENT_ID[encounters_first_hep_c_encounter$before_diagnosis=="TRUE" & encounters_first_hep_c_encounter$Enc_EIO %in% c("O", "I, O", "E, O", "E, I, O")]))
...
patient_ID_encounters_inpt <- data.frame(table(encounters_first_hep_c_encounter$PATIENT_ID[encounters_first_hep_c_encounter$before_diagnosis=="TRUE" & encounters_first_hep_c_encounter$Enc_EIO %in% c("I", "I, O", "E, I, O", "E, I")]))
...
#Average_LOS
#aggregate(s_encounters_first_hep_c_encounter$LOS_Days, by = list(PATIENT_ID=s_encounters_first_hep_c_encounter$PATIENT_ID), FUN=sum)
...
#Total_Num_ED_Visits
patient_ID_encounters_ED <- data.frame(table(encounters_first_hep_c_encounter$PATIENT_ID[encounters_first_hep_c_encounter$before_diagnosis=="TRUE" & encounters_first_hep_c_encounter$Enc_EIO %in% c("E, I", "E", "E, I, O", "E, O")]))
...
patient_ID_insurance_aided <- data.frame(table(encounters_first_hep_c_encounter$PATIENT_ID[encounters_first_hep_c_encounter$before_diagnosis=="TRUE" & encounters_first_hep_c_encounter$Fin_Class == "Aided"]))
...
patients[,"InsuranceAidedCt"][is.na(patients[,"InsuranceAidedCt"])] <- 0
patients[,"Insurance_Aided"][is.na(patients[,"Insurance_Aided"])] <- "FALSE"


#Insurance_Gap
patient_ID_insurancegap <- data.frame(table(encounters_first_hep_c_encounter$PATIENT_ID[encounters_first_hep_c_encounter$before_diagnosis=="TRUE" & (is.na(encounters_first_hep_c_encounter$Fin_Class)| encounters_first_hep_c_encounter$Fin_Class == "Self Pay")]))
colnames(patient_ID_insurancegap) <-  c("PATIENT_ID", "InsuranceGap")
...
patients[,"InsuranceGap"][is.na(patients[,"InsuranceGap"])] <- 0
patients[,"Insurance_Gap"][is.na(patients[,"Insurance_Gap"])] <- "FALSE"

#Zip_Code
#encounters_minus_NAzips <- encounters_first_hep_c_encounter[!is.na(encounters_first_hep_c_encounter$Zip_Code_DID),]
